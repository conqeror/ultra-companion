...
query FerryTrips($from: String!, $to: String!, $dateTime: DateTime!) {
  trip(
    from: { place: $from }
    to: { place: $to }
    dateTime: $dateTime
...
export function parseEnturFerryDepartures(payload: unknown, after: Date): FerryDeparture[] {
  ...
  for (const patternValue of patterns) {
    ...
    for (const legValue of legs) {
      ...
      if (departureMs == null || departureMs < afterMs) continue;
...
export async function fetchEnturFerryDepartures(
  ...
  const departures = parseEnturFerryDepartures(payload, after);
