...
- Opening hours awareness — highlight POIs that are currently open, dim closed ones
...
- **Race as a route collection** — a race is an ordered set of GPX segments (e.g., 4000km race split into ~200km files). Group and order them as one race. The app shows a continuous window around the rider's position regardless of GPX file boundaries.
