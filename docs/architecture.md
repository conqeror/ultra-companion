...
Recurring timetables are intentionally not modeled or persisted. A user can explicitly match the saved boarding and landing coordinates to nearby Entur water `StopPlace` records; their directional IDs and display names are stored as opaque `providerRefs`. Upcoming asks Entur Journey Planner for concrete trips after the quay ETA plus boarding buffer, keeps results in memory for five minutes, and shows the first usable water departure and arrival. The live result refines that ferry row only: downstream route ETA remains deterministic from the saved boarding buffer, assumed wait, and crossing duration, which also remain the offline/error fallback.
