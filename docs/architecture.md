...
Recurring timetables are intentionally not modeled or persisted. A user can explicitly match the saved boarding and landing coordinates to nearby Entur water `StopPlace` records; their directional IDs and display names are stored as opaque `providerRefs` and provide the canonical `boarding – landing` display name. Upcoming asks Entur Journey Planner for concrete directional trips around the quay ETA plus boarding buffer, keeps the forward result in memory for five minutes, and shows the first usable water departure and arrival in the collapsed row. Tapping the row exposes the previous departure only when it falls within the preceding hour, the next five departures, and an explicit last departure for that local day. The live result refines that ferry row only: downstream route ETA remains deterministic from the saved boarding buffer, assumed wait, and crossing duration, which also remain the offline/error fallback.
