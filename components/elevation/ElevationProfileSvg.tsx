...
interface ElevationProfileSvgProps extends ElevationProfileProps {
  /** Internal limit used by fail-open renderers that must stay bounded. */
...
export default function ElevationProfile({
  ...
  const detailInterval = useMemo(() => {
    if (!isScrollable) {
      return detailSampleLimit == null
        ? BASE_INTERVAL_M
        : getElevationIntervalForSampleLimit(totalMeters, detailSampleLimit, BASE_INTERVAL_M);
    }
    ...
    const byCap = getElevationIntervalForSampleLimit(
      totalMeters,
      detailSampleLimit ?? MAX_DETAIL_SAMPLES,
    );
    return Math.max(BASE_INTERVAL_M, byDensity, byCap);
  }, [detailSampleLimit, isScrollable, totalMeters, innerWidth]);
